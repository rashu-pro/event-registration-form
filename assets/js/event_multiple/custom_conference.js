...
$('.cart-table .quantity-increase').on('click', function () {
    let self = $(this),
...
$('.cart-table .quantity-decrease').on('click', function () {
    let self = $(this),
        quantitySelector = self.closest('.quantity-wrap').find('.form-control'),
        quantityValue = quantitySelector.val();
...
//=== ADDON TICKET QUANTITY INCREASE BUTTON CLICK EVENT
$(document).on('click', '.addon-ticket .quantity-increase', function () {
    let self = $(this),
        dataRow = self.closest('.form-row-ticket-individual').data('row'),
        quantitySelector = self.closest('.quantity-wrap').find('.form-control'),
        quantityValue = quantitySelector.val(),
        ticketType = quantitySelector.data('type'),
        uniqueId = new Date().getUTCMilliseconds();

    quantitySelector.val(parseInt(quantityValue) + 1);

    let ticketText = quantitySelector.data('name'),
        unitPrice = quantitySelector.data('unitprice'),
        price = parseInt(unitPrice)*parseInt(quantitySelector.val()),
        row = `<tr id='row-addon-${ticketType}-${dataRow}' class='price-row ticket-price-row addon-price-row price-addon-${ticketType}-${dataRow}' data-quantity="${quantitySelector.val()}" data-row="${dataRow}">
    ...
    $('.ticket-summary-table .price-addon-'+ticketType+'-'+dataRow).remove();
    if(quantitySelector.val()>0) $('.ticket-summary-table table tbody').append(row);
...
//=== ADDON TICKET QUANTITY INCREASE BUTTON CLICK EVENT
$(document).on('click', '.addon-ticket .quantity-decrease', function () {
    let self = $(this),
        dataRow = self.closest('.form-row-ticket-individual').data('row'),
        quantitySelector = self.closest('.quantity-wrap').find('.form-control'),
        quantityValue = quantitySelector.val(),
    ...
    if(quantityValue>0) quantitySelector.val(parseInt(quantityValue) - 1);
...
//=== ADDON TICKET QUANTITY FIELD KEYUP, FOCUS, BLUR EVENT
$(document).on('click', '.addon-ticket .form-control', function () {
    let self = $(this),
        dataRow = self.closest('.form-row-ticket-individual').data('row'),
        quantitySelector = self,
        quantityValue = quantitySelector.val(),
        ticketType = quantitySelector.data('type'),
        uniqueId = new Date().getUTCMilliseconds();
    ...
    $('.ticket-summary-table .price-addon-'+ticketType+'-'+dataRow).remove();
    if(parseInt(quantitySelector.val())>0) $('.ticket-summary-table table tbody').append(row);

    calculateTotal();
    calculateCoupon($('.btn-apply-voucher-js'), false);
});

$('.cart-table .quantity-wrap .form-control').on('keyup', function () {
    let self = $(this);
    collectData(self);
    calculateCoupon($('.btn-apply-voucher-js'), false);
});
...
$(document).on('change', '.ticket-type-js', function (e) {
    ...
    //=== if addon ticket is already selected in cart
    mainParent.find('.addon-ticket .form-control').val(0);
    $('.ticket-summary-table .addon-price-row').each(function (i, element) {
        if($(element).data('row')==dataRow){
            $(element).remove();
        }
    });
...
function calculateGrandTotal() {
    let totalPrice = parseInt($('.ticket-summary-table .total-price .amount').html() | 0),
        voucherPrice = parseInt($('.ticket-summary-table .voucher-price .amount').html() | 0),
    ...
    $('#CustomerEnteredPrice').val(totalPrice);
...
$('#CustomerEnteredPrice').on('keyup', function () {
    let self = $(this);
    let totalPrice = parseInt(self.val() | 0),
        voucherPrice = parseInt($('.ticket-summary-table .voucher-price .amount').html() | 0),
        grandTotalPrice = totalPrice - voucherPrice;
